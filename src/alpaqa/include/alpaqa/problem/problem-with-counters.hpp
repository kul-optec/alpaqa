...
struct ProblemWithCounters {
    ...
    [[gnu::always_inline]] void eval_prox_jacobian_diag(real_t γ, crvec x, rvec J_diag) const requires requires { &std::remove_cvref_t<Problem>::eval_prox_jacobian_diag; } { ++evaluations->prox_jacobian_diag; return timed(evaluations->time.prox_jacobian_diag, [&] { return problem.eval_prox_jacobian_diag(γ, x, J_diag); }); }
    [[gnu::always_inline]] real_t eval_nonsmooth_objective(crvec x) const requires requires { &std::remove_cvref_t<Problem>::eval_nonsmooth_objective; } { ++evaluations->nonsmooth_objective; return timed(evaluations->time.nonsmooth_objective, [&] { return problem.eval_nonsmooth_objective(x); }); }
    ...
    [[nodiscard]] bool provides_eval_prox_jacobian_diag() const requires requires (Problem p) { { p.provides_eval_prox_jacobian_diag() } -> std::convertible_to<bool>; } { return problem.provides_eval_prox_jacobian_diag(); }
    [[nodiscard]] bool provides_eval_nonsmooth_objective() const requires requires (Problem p) { { p.provides_eval_nonsmooth_objective() } -> std::convertible_to<bool>; } { return problem.provides_eval_nonsmooth_objective(); }
