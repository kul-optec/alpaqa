...
void print_usage(const char *a0, std::ostream &os) {
    const auto *opts = " [<problem-type>:][<path>/]<name> [method=<solver>] "
                       "[<key>=<value>...]\n";
    os << "alpaqa-driver " ALPAQA_VERSION_FULL " (" << alpaqa_build_time
       << ")\n\n"
...
void print_version(std::ostream &os) {
    os << ALPAQA_VERSION_FULL " (" << ALPAQA_BUILD_TIME << ")\n";
}

SolverBuilders::SolverBuilders(std::string default_solver)
    : default_solver(std::move(default_solver)),
      solvers{
...
SolverBuilders::get_solver_builder(Options &opts) const {
    std::string method = default_solver, direction;
    ...
    return std::make_tuple(solver_it->second, direction);
}

void SolverBuilders::register_solver_builder(std::string_view name,
                                             solver_builder_func builder) {
    solvers.insert({name, std::move(builder)});
}
...
std::ostream &get_output_stream(alpaqa::Options &opts,
    ...
    return out_fstream.is_open() ? out_fstream : default_stream;
}
...
std::tuple<fs::path, std::string_view>
...
std::string store_solution(const fs::path &sol_output_dir, std::ostream &os,
    ...
    for (auto [name, fname, value] : solutions) {
        ...
        if (!warm_start_cmd.empty())
            warm_start_cmd += " ";
        warm_start_cmd += "'";
        warm_start_cmd += fname;
        warm_start_cmd += "0=@";
        warm_start_cmd += pth.string();
        warm_start_cmd += "'";
    }
    {
...
#endif
    {
        auto pth = sol_output_dir / ("eval" + suffix + ".yml");
        os << "Writing evaluation counters to " << pth << std::endl;
        std::ofstream output_file(pth);
        write_evaluations(output_file, sol_res.evals);
    }
    if (!warm_start_cmd.empty()) {
        os << "To warm start using this solution, use: " << warm_start_cmd
           << std::endl;
    }
    return suffix;
}
