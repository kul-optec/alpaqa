...
def main():
    # alpaqa
    parser = argparse.ArgumentParser(description="alpaqa cache operations", allow_abbrev=False)
    parser.add_argument(
        "--version",
        "-v",
        action="store_true",
    ...
    cleanparser = cachesubparsers.add_parser(
        "clean",
        description="clean up all cached files",
    )
    ...
    if args.version:
        from .alpaqa import build_time, commit_hash  # noqa: PLC0415
